...
header = '\n'.join(
    [
        ...
        f'R_out : {R_out}',
        f'R_in  : {R_in}',
        f'diameter_beam_root:{dia_beam_root}',
        f'diameter_beam_tip:{dia_beam_tip}',
        f'r_beam_root:{r_beam_root}',
        f'r_beam_tip:{r_beam_tip}',
        f'beam_length:{beam_length}',
        f'Thickness:{Thickness}',
        f'RIB_MARGIN:{RIB_MARGIN}', 
        f'N     : {N}',
        f'M     : {M}',
        f'dt    : {dt}',
        f'thrust: {thrust}',
...
beam_spline=interpolate.interp1d(
    r,dia_beam,kind='linear',fill_value='extrapolate'
    )
beam_spline_list=beam_spline(r_spline)

header = ['半径r[mm]','翼弦長chord[mm]','取付角theta[deg]','桁径[m]']
...
with open(filename,'w',newline="") as f:
    ...
    for i in range(len(r_spline)):
        writer.writerow(
            [r_spline[i], chord_spline_list[i], theta_spline_list[i],beam_spline_list[i]]
            )
...
plt.subplot(3,3,1)
plt.plot(r, chord)
plt.plot(r, (dia_beam+RIB_MARGIN)/Thickness)

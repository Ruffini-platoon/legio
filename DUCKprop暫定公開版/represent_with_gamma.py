...
#Re*Cl(式39)
F_ReCl = lambda gamma: (2*F_L(gamma))/(RHO*NU*F_V(gamma)*delta_B*nc_Re)

#揚力係数(式14,21,44)
F_CL = lambda gamma: optimaized_cl(F_ReCl(gamma))

#レイノズル数(式45)
F_Re = lambda gamma: (F_ReCl(gamma)*nc_Re)/F_CL(gamma)

#抗力係数(式15,22,47)
F_CD = lambda gamma: optimaized_CDRe(F_ReCl(gamma))/F_Re(gamma)

#翼弦長(式16,23,48)
F_C = lambda gamma: (NU*F_ReCl(gamma)*nc_Re)/(F_V(gamma)*F_CL(gamma))

#迎角(式17,24,49)
F_alpha = lambda gamma: dae51.alpha_ReCL(F_Re(gamma) ,F_CL(gamma))

# gammaのみの関数としてのdD(式10, 25)
F_D = lambda gamma: (1/2) * RHO * F_V(gamma)**2 * F_C(gamma) * delta_B * F_CD(gamma)

...
def Robust_analysis_at( U_inf , RPM ):
    ...
    if numpy.isnan(F_T(f1gamma)):#初期値その1で推力がnanになったら初期値その2へ
        ...
        if numpy.isnan(F_T(f2gamma)):#初期値その２でもダメならあきらめる
            ...
            return numpy.nan , numpy.nan , numpy.nan , numpy.nan #返すのがnanではなく0だと上手くいかない。なぜまぜ
            ...
            print(F_eta(result.x)*100,' %') #推力が負になったりすると定義できない。ありえない数字（100以上や負）が出た時はそれ。
            print('Norm: ',numpy.linalg.norm( F_gamma(result.x) - (result.x) ))#ノルム。結果の精度の指標。1以下だとうれしい。大きい時は局所解に落ちていると思われる。改善求ム

            return F_T(result.x) , F_P(result.x) , F_eta(result.x) , numpy.linalg.norm( F_gamma(result.x) - (result.x) )

    else:
        result = minimize(
            Object,
            x0=f1gamma,
            options={"maxiter":1e8,"ftol":1e-3},#ftolは1e-3くらいが良い
            method='SLSQP'
        )
        print('complete')
        #print(result) #status(収束しているか)の確認用
